                ...
                <?php foreach (WC()->cart->cart_contents as $cart_item_key => $cart_item) { ?>
                    ...
                    <li class="item clearfix">
                        <?php if ( ! $_product->is_visible() ) { ?>
                            <?php echo '<div class="product-image">' . $_product->get_image().'</div>'; ?>
                        <?php } else { ?>
                        ...
                        <div class="product-details">
                            ...
                            if ( ! $_product->is_visible() ) {
                                echo '<div class="product-name">' . apply_filters('woocommerce_cart_widget_product_title', $product_title, $_product) . '</div>';
                            } else {
                            ...
                            echo '<span class="price">'.wc_price($_product->get_price()).'</span>';
                            ?>
                        </div>
        ...
        <div class="add-links<?php if ( $wishlist && $compare ) echo ' show-all' ?>">
            ...
            <?php if (defined('ADDTHIS_INIT') && $venedor_settings['product-addthis']) : // addthis buttons ?>
                <div class="addthis-icons clearfix">
                    <?php if (isset($addthis_options) && isset($addthis_options['addthis_for_wordpress']) && ($addthis_options['addthis_for_wordpress'] == true)) {
                        echo '<div data-url="'.get_permalink().'" data-title="'.get_the_title().'" class="at-above-post addthis_toolbox"></div>';
                    } else {
                        ...
                        do_action('addthis_widget', get_permalink(), get_the_title(), 'above'); add_filter('addthis_above_content', 'venedor_addthis_remove', 10, 1);
                    } ?>
                </div>
            <?php endif; ?>
        </div>
...
function venedor_layered_nav_link($link) {
    ...
    if (!empty($params['product_orderby']))
        $link = add_query_arg( 'product_orderby', $params['product_orderby'], $link );

    if (!empty($params['product_order']))
        $link = add_query_arg( 'product_order', $params['product_order'], $link );

    if (!empty($params['product_count']))
        $link = add_query_arg( 'product_count', $params['product_count'], $link );
